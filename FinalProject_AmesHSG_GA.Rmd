...
findAvg<-function(x){
  if(typeof(x) == "character" || typeof(x)=="integer"){
...
gam.fit <- gam(SalePrice ~ bs(Gr.Liv.Area, 4) + ns(Overall.Qual, 3) + s(X1st.Flr.SF, 7) + MS.SubClass +
                 bs(X2nd.Flr.SF, 6) + poly(as.numeric(BsmtFin.Type.1), 5) + MS.Zoning + poly(Full.Bath, 3) + 
                 poly(Kitchen.Qual, 2) + Neighborhood + Central.Air, data = housing.train)

gam.fit1 <- gam(SalePrice ~ bs(Gr.Liv.Area, 4) + ns(Overall.Qual,3) + s(X1st.Flr.SF, 10) + MS.SubClass +
...
```{r}
##Ingest informaiton from User to set baseline undertanding of home & understanding of type of renovations they desire to make
# Items ask directly correspond to variables considered to be meaningful in predicting home value
...
writeLines("Blmngtn.	Bloomington Heights
Blueste.	Bluestem
BrDale.	Briardale
BrkSide.	Brookside
ClearCr.	Clear Creek
...
Timber.	Timberland
Veenker.	Veenker")
neighborhood <- readline("Enter the code from the list above: ") #For some reason this isn't reading in, but feel free to adjust

lot.area <- readline("What is total square footage of your lot?: ")
lot.area <- as.double(lot.area)
writeLines("Are you looking to increase it's size(i.e. purchase adjacent property?")
lot.area.upgrd <- readline("If yes enter the square-footage of your intended design.  If no enter 0: ")
lot.area.upgrd <- as.double(lot.area.upgrd)


writeLines("Describe your house for us?")
...
180.	PUD - MULTILEVEL - INCL SPLIT LEV/FOYER
190.	2 FAMILY CONVERSION - ALL STYLES AND AGES")
ms.class <- readline("Enter the number from the list above: ")
ms.class <- as.double(ms.class)
writeLines("Are you looking to change to the following features?")
writeLines("40	1-STORY W/FINISHED ATTIC ALL AGES
45.	1-1/2 STORY - UNFINISHED ALL AGES
50.	1-1/2 STORY FINISHED ALL AGES 
75.	2-1/2 STORY ALL AGES
80.	SPLIT OR MULTI-LEVEL
...
180.	PUD - MULTILEVEL - INCL SPLIT LEV/FOYER
190.	2 FAMILY CONVERSION - ALL STYLES AND AGES")
ms.class.upgrd <- readline("If yes, enter the number from the list above.  If no enter 0: ")
ms.class.upgrd <- as.double(ms.class.upgrd)

writeLines("How is your house zoned?")
writeLines("A (agr)		Agriculture
C (all)		Commercial
...
zone <- readline("Enter the code from the list above: ")

writeLines("What type of utilities does your home have?")
writeLines("AllPub	All public Utilities (E,G,W,& S)	
NoSewr	Electricity, Gas, and Water (Septic Tank)
NoSeWa	Electricity and Gas Only")
utilities <- readline("Enter the code from the list above: ")
utilities.upgrd <- readline("Are you looking to add additional utitilieis?  If yes enter the code of what your end goal utilities are.  If no enter `None`: ")


x1st.sf <- readline("What is the square footage of the first level in your house?: ")
x1st.sf <- as.double(x1st.sf)
x1st.sf.upgrd <- readline("Are you looking to increase it's size?  If yes enter the square-footage of your intended design.  If no enter 0: ")
x1st.sf.upgrd <- as.double(x1st.sf.upgrd)


liv.area <- readline("What is the square footage of your ground-level living room?: ")
liv.area <- as.double(liv.area)
liv.area.upgrd <- readline("Are you looking to increase it's size?  If yes enter the square-footage of your intended design.  If no enter 0: ")
liv.area.upgrd <- as.double(liv.area.upgrd)


x2st.sf <- readline("What is the square footage of the second level in your house?: ")
x2st.sf <- as.double(x2st.sf)
x2st.sf.upgrd <- readline("Are you looking to increase it's size?  If yes enter the square-footage of your intended design.  If no enter 0: ")
x2st.sf.upgrd <- as.double(x2st.sf.upgrd)


writeLines("What would you say is the overall quality of your home?")
writeLines("10.     Very Excellent \n 9.	Excellent \n 8.	Very Good \n 7.	Good \n 6.	Above Average \n 5.	Average \n 4.	Below Average \n 3.	Fair  \n 2.	Poor  \n 1.	Very Poor")
overall.qual <- readline("Enter the number from the list above: ")
overall.qual <- as.double(overall.qual)
overall.qual.upgrd <- readline("Are you looking to improve the quality?  If yes, enter the number from the list above.  If no enter 0: ")
overall.qual.upgrd <- as.double(overall.qual.upgrd)


writeLines("What would you say is the overall quality of your kitchen?")
writeLines("5. Ex	Excellent
4. Gd	Good
3. TA	Typical/Average
2. Fa	Fair
1. Po	Poor")
ktchn.qual <- readline("Enter the code from the list above: ")
ktchn.qual <- as.integer(ktchn.qual)
ktchn.qual.upgrd <- readline("Are you looking to improve the quality of your kitchen?\nIf yes, enter the target quality from the list.  If no enter `None`: ")
ktchn.qual.upgrd <- as.integer(ktchn.qual.upgrd)

writeLines("What would you say is the overall quality of your basement (if you have one) with respect to ceiling height")
writeLines("5. Ex	Excellent (100+ inches)	
4. Gd	Good (90-99 inches)
3. TA	Typical (80-89 inches)
2. Fa	Fair (70-79 inches)
1. Po	Poor (<70 inches)
None	No Basement")
bsmt.qual <- readline("Enter the code from the list above: ")
bsmt.qual <- as.integer(bsmt.qual)
bsmt.qual.upgrd <- readline("Are you looking to improve the height of your basement?\nIf yes, enter the target grade from the list.  If no enter `None`: ")


full.bath <- readline("How many full baths do you have on the first and/or second floors of our home?")
full.bath <- as.integer(full.bath)
full.bath.upgrd <- readline("Are you looking to increase the number of full baths on these floors?\nIf yes enter the number of bathrooms you wish to add.  If no enter 0: ")
full.bath.upgrd <- as.integer(full.bath.upgrd)

home.details <- data.frame(Gr.Liv.Area = liv.area, Gr.Liv.Area.upgrd = liv.area.upgrd, 
                           Overall.Qual = overall.qual, overall.qual.upgrd = overall.qual.upgrd, 
                           X1st.Flr.SF = x1st.sf, x1st.sf.upgrd = x1st.sf.upgrd, MS.SubClass = ms.class, 
                           ms.class.upgrd = ms.class.upgrd, X2nd.Flr.SF = x2st.sf, x2st.sf.upgrd = x2st.sf.upgrd, 
                           Bsmt.Qual = bsmt.qual,bsmt.qual.upgrd = bsmt.qual.upgrd, MS.Zoning = zone,
                           Full.Bath = full.bath,full.bath.upgrd = full.bath.upgrd,Kitchen.Qual = ktchn.qual,
                           ktchn.qual.upgrd = ktchn.qual.upgrd,Neighborhood = neighborhood,Utilities = utilities,
                           utilities.upgrd = utilities.upgrd,Lot.Area = lot.area,lot.area.upgrd = lot.area.upgrd)
return(home.details)
}
```
...
```{r}
##Function that outputs dataframe containing user-input baseline home data and user-input upgrade requests
#Function checks for NULL values (i.e. user doesn't want an upgrae): 0/'None' and updates the dataframe accordingly

process.baseline <- function(home.baseline = NULL){
  home.upgrade <- home.baseline
  home.upgrade$Gr.Liv.Area <- home.upgrade$liv.area.upgrd
  home.upgrade$Overall.Qual <- home.upgrade$overall.qual.upgrd
  home.upgrade$X1st.Flr.SF <- home.upgrade$x1st.sf.upgrd
  home.upgrade$MS.SubClass <- home.upgrade$ms.class.upgrd
  home.upgrade$X2nd.Flr.SF <- home.upgrade$x2st.sf.upgrd
  home.upgrade$Bsmt.Qual <- home.upgrade$bsmt.qual.upgrd
  home.upgrade$Full.Bath <- home.upgrade$Full.Bath + home.upgrade$full.bath.upgrd #be careful how you reassign values as per question asked
  home.upgrade$Kitchen.Qual <- home.upgrade$ktchn.qual.upgrd
  home.upgrade$Utilities <- home.upgrade$utilities.upgrd
  home.upgrade$Lot.Area <- home.upgrade$lot.area.upgrd
return(home.upgrade) #
}
```
...
#Generate baseline dataframe from information per user input
home.baseline <-info.ingest()
#Generate upgrade information from user input
home.upgrade <- process.baseline(home.baseline)

#Calculate baseline predicted home value per user input
gam.baseline.pred <- predict(gam.fit, newdata = home.baseline)

#Calculate predicted upgraded home value per user input
gam.upgrade.pred <- predict(gam.fit, newdata = home.upgrade)

#Calculate House Value Delta
value.delta <- gam.upgrade.pred - gam.baseline.pred

percent.increase <- value.delta/gam.baseline.pred

message <- paste("The changes you have proposed to make in your home upgrade have increased it's value by $", value.delta, "which is a", percent.increase,"% increase in value")
