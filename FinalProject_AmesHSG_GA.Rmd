...
```{r}
basement.cv <- smoothCV(x = housing.train$Total.Bsmt.SF,
         y = housing.train$SalePrice,
         K = 10,
         df.min = 1,
         df.max = 6)

plot.smoothCV(basement.cv, K = 10, title.text = "CV Error: SalePrice ~ Total.Bsmt.SF",
              y.scale.factor = 1.1)
```


<font color="#800000">
We will use a smoothing spline with degree 5 for Total.Bsmt.SF
</font>
...
```{r}
gam.fit <- gam(SalePrice ~ bs(Gr.Liv.Area, 4) + ns(Overall.Qual,3) + s(X1st.Flr.SF, 6) + MS.SubClass +
                 poly(X2nd.Flr.SF, 3) + s(Total.Bsmt.SF, 5) + MS.Zoning + poly(Full.Bath, 3) + 
                 Kitchen.Qual + Neighborhood + Central.Air, data = housing.train)
summary(gam.fit)
