...
getbaselineRow<-function(subclass, neighborhood, value){
   sub.indexes<-which(housing.train$MS.SubClass == subclass)
   nb.indexes<- which(housing.train$Neighborhood == neighborhood)
   val.indexes <- which(housing.train$SalePrice >= value - 10000 & housing.train$SalePrice <= value + 10000)
...
```{r}
y = readline(prompt="Input variable ")
typeof(y)
