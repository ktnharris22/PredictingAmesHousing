...
avgRow <- sapply(housing.test, findAvg)
avgRow <- data.frame(avgRow)
...
writeLines("If yes, enter the additional square-footage of your intended increase (do not including current lot area). 
           If no, enter '0'.")
lot.area.upgrd <- readline("Input: ")
lot.area.upgrd <- as.numeric(lot.area.upgrd)
...
ms.class <- readline("Input: ")
ms.class <- as.integer(ms.class)
...
180.	PUD - MULTILEVEL - INCL SPLIT LEV/FOYER
190.	2 FAMILY CONVERSION - ALL STYLES AND AGES")
writeLines("Are you looking to change your home design to one of the designs above?")
writeLines("If yes, enter the number from the list above.  If no enter '0'.")
ms.class.upgrd <- readline("Input: ")
ms.class.upgrd <- as.integer(ms.class.upgrd)
if(ms.class.upgrd == 0) {
  ms.class.upgrd <- ms.class
}
...
writeLines("Are you looking to increase it's size?
If yes, enter the approximate square footage of the NEW addition's intended size.
If you do not want to upgrade, enter '0'.")
liv.area.upgrd <- readline("Input: ")
liv.area.upgrd <- as.numeric(liv.area.upgrd)
...
x2st.sf <- readline("Input: ")
x2st.sf <- as.numeric(x2st.sf)
writeLines("Are you looking to increase it's size?
If yes, enter the approximate square-footage of the NEW addtion's intended size.
If you do not want to upgrade, enter 0.")
x2st.sf.upgrd <- readline("Input: ")
x2st.sf.upgrd <- as.numeric(x2st.sf.upgrd)
...
overall.qual <- as.integer(overall.qual)

writeLines("\n")

writeLines("Are you looking to improve the overall quality of your home?
If yes, enter the number from the list above you want your home to be at.
If no enter '0'.")
overall.qual.upgrd <- readline("Input: ")
overall.qual.upgrd <- as.numeric(overall.qual.upgrd)
...
# if(!toupper(bsmt.qual.upgrd) == "NONE") {
#   bsmt.qual.upgrd <- as.integer(bsmt.qual.upgrd)
# }
...
NoSewr --- Electricity, Gas, and Water (Septic Tank)
NoSeWa --- Electricity and Gas Only")
writeLines("Are you looking to add or change utilities?
If yes, enter the code for the utilities are from the list above. If no enter `None`.")
utilities.upgrd <- readline("Input: ")
if(toupper(utilities.upgrd) == "NONE") {
  utilities.upgrd <- utilities
} else {
    if(toupper(utilities.upgrd) == "NOSEWR") {
      utilitie.upgrd <- "NoSewr"
    } else if(toupper(utilities.upgrd) == "NOSEWA") {
      utilities.upgrd <- "NoSeWa"
    } else {
      utilities.upgrd <- "AllPub"
    }
}
...
ac <- readline("Input: ")
ac <- toupper(ac)
writeLines("Are you looking to add central air conditioning to your home?
Enter 'Y' if yes, or 'N' if no.")
ac.upgrd <- readline("Input: ")
if(toupper(ac.upgrd) == "N") {
  ac.upgrd <- ac
} else {
  ac.upgrd <- "Y"
}

home.details <- data.frame(Year.Built = year,
                           Gr.Liv.Area = liv.area, liv.area.upgrd = liv.area.upgrd, 
                           Overall.Qual = overall.qual, overall.qual.upgrd = overall.qual.upgrd,
                           Exter.Qual = exter.qual,
                           X1st.Flr.SF = x1st.sf, x1st.sf.upgrd = x1st.sf.upgrd,
                           MS.SubClass = ms.class, 
                           ms.class.upgrd = ms.class.upgrd, 
                           X2nd.Flr.SF = x2st.sf, x2st.sf.upgrd = x2st.sf.upgrd, 
                           Bsmt.Qual = bsmt.qual, BsmtFin.Type.1 = basement,
                           MS.Zoning = zone,
                           Fireplaces = fire, Fireplace.Qu = fire.qual,
                           Full.Bath = full.bath, full.bath.upgrd = full.bath.upgrd,
                           Half.Bath = half.bath, half.bath.upgrd = half.bath.upgrd,
                           Central.Air = ac, ac.upgrade = ac.upgrd,
                           Kitchen.Qual = ktchn.qual, ktchn.qual.upgrd = ktchn.qual.upgrd,
                           Neighborhood = neighborhood,
                           Garage.Cars = garage, Garage.Type = garage.type, Garage.Area = garage.area,
                           Utilities = utilities, utilities.upgrd = utilities.upgrd,
                           Lot.Area = lot.area,lot.area.upgrd = lot.area.upgrd)
return(home.details)
}
```
...
process.baseline <- function(home.baseline = NULL){
  ...
  home.upgrade$Lot.Area <- home.upgrade$Lot.Area + home.upgrade$lot.area.upgrd
  home.upgrade$Gr.Liv.Area <- home.upgrade$Gr.Liv.Area + home.upgrade$liv.area.upgrd
  home.upgrade$Overall.Qual <- home.upgrade$overall.qual.upgrd
  home.upgrade$X1st.Flr.SF <- home.upgrade$X1st.Flr.SF + home.upgrade$x1st.sf.upgrd
  home.upgrade$X2nd.Flr.SF <- home.upgrade$X2nd.Flr.SF + home.upgrade$x2st.sf.upgrd
  ...
  home.upgrade$Half.Bath <- home.upgrade$Half.Bath + home.upgrade$half.bath.upgrd
  home.upgrade$Kitchen.Qual <- home.upgrade$ktchn.qual.upgrd
  home.upgrade$Utilities <- home.upgrade$utilities.upgrd
  home.upgrade$Central.Air <- home.upgrade$ac.upgrd
return(home.upgrade) 
}
```
