...
baseline["Year.Built"] = year
baseline["Gr.Liv.Area"] = liv.area
baseline["Overall.Qual"] = overall.qual
baseline["Exter.Qual"] = exter.qual
baseline["X1st.Flr.SF"] = x1st.sf
baseline["MS.SubClass"] = ms.class
baseline["X2nd.Flr.SF"] = x2nd.sf
baseline["Bsmt.Qual"] = bsmt.qual
baseline["BsmtFin.Type.1"] = basement
baseline["MS.Zoning"] = zone
baseline["Fireplaces"] = fire
baseline["Fireplace.Qu"] = fire.qual
baseline["Full.Bath"] = full.bath
baseline["Half.Bath"] = half.bath
baseline["Central.Air"] = ac
baseline["Kitchen.Qual"] = ktchn.qual
baseline["Neighborhood"] = neighborhood
baseline["Garage.Cars"] = garage
...
gam.baseline.pred <- predict(gam.fit, newdata = baseline)

#Calculate predicted upgraded home value per user input
gam.upgrade.pred <- predict(gam.fit, newdata = renovated)
...
return.list <- list(gam.baseline.pred, gam.upgrade.pred, message)
