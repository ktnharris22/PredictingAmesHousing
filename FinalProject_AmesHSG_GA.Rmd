...
Variables to use: Gr.Liv.Area, Overall.Qual, X1st.Flr.SF, MS.SubClass, X2nd.Flr.SF, BsmtFin.Type.1, MS.Zoning, Full.Bath, Kitchen.Qual, TotRms.AbvGrd, Neighborhood, Central.Air
...
```{r}
#took this out
#rooms.cv <- smoothCV(x = housing.train$TotRms.AbvGrd,
#         y = housing.train$SalePrice,
#        K = 10,
#         df.min = 1,
#         df.max = 10)

#plot.smoothCV(rooms.cv, K = 10, title.text = "CV Error: SalePrice ~ TotRms.AbvGrd",
#              y.scale.factor = 1.1)
```
...
```{r}
basement.cv <- smoothCV(x = as.numeric(housing.train$BsmtFin.Type.1),
         y = housing.train$SalePrice,
         K = 10,
         df.min = 1,
         df.max = 6)

plot.smoothCV(basement.cv, K = 10, title.text = "CV Error: SalePrice ~ BsmtFin.Type.1",
              y.scale.factor = 1.1)
```
...
Our GAM model accounts for roughly 90.3% of the deviance explained. 
